    ...
    $.get("api_menu.php",
        function(data){
            ...
            for (g in menuarr){
                ...
                txt=txt+"<li id=group><a href='#' id=group"+g+">"+groupName+"</a>";
                txt=txt+"<ul>"; 
                for (tp in menuarr[g].dataTypes){
                    typeName=menuarr[g].dataTypes[tp].typeName;
                    typeCode=typeName.replace(/ /g, "_");
                    txt=txt+"<li><a href='#' id=type"+tp+">"+typeName+"</a>";
                    txt=txt+"<ul>";
                    //alert(type);
                    for (d in menuarr[g].dataTypes[tp].datasets){
                        datasetName=menuarr[g].dataTypes[tp].datasets[d].datasetName;
                       // alert(datasetName);
                        txt=txt+"<li><input type=checkbox id="+g+"-"+d+"-"+typeCode+" onClick=showhideDataset(\""+g+"\",'"+d+"','"+typeCode+"')><a href='#'>"+datasetName+"</a></li>";
                    }
                    txt=txt+"</ul>";
                }
                txt=txt+"</li>";
